...
export const SubMenu = ({menu, pathname, subMenuItem = false, mobile = false, close={close}}) => {
    const absolute = mobile ? 'relative' : 'absolute top-16';

    return (
        <ul className={`sub-menu ${absolute} pt-2 pb-4`}>
            {menu.map((el) => {
                return <NavLink el={el} key={el.link} pathname={pathname} subMenuItem={true} close={close}/>
            })}
        </ul>
    );
}

export const NavLink = ({el, pathname, subMenuItem = false, mobile = false, close = () => {}}) => {
    const active = activeLinkClass(el.link, subMenuItem, el.subMenu, pathname);
    const subItem = subMenuItem ? 'sub-menu-item text-md' : 'main-nav-element text-lg';
    ...
    return (
        <li 
            ...
            <div className="flex flex-row">
                <Link href={el.link} className={linkClasses} onClick={() => close()}>
                    {el.name}
                </Link>
                {el.subMenu && mobile && <ChevronDownIcon 
                                        onClick={() => setToggle(!toggle)} 
                                        className="ms-1 w-4 h-4 cursor-pointer self-center" />}                
            </div>

            {toggle && el.subMenu && <SubMenu menu={el.subMenu} pathname={pathname} mobile={mobile} close={close} />}
        </li>
    );
}